...
public class CrearReporteVentas extends JDialog implements ActionListener {
    ...
    public CrearReporteVentas(AplicationContext context, Dahsboard dahsboard) {
        ...
        {
            JLabel lbFecha = new JLabel("Ingrese la fecha: (YYYY-MM-DD)");
            lbFecha.setFont(Fonts.custom);
            lbFecha.setForeground(Color.WHITE);
            lbFecha.setBounds(20, 97, 200, 23);
            contentPanel.add(lbFecha);
        }
        {
            textFieldlbFecha = new JTextField();
            textFieldlbFecha.setBounds(20, 127, 301, 26);
            contentPanel.add(textFieldlbFecha);
            textFieldlbFecha.setColumns(10);
        }
        {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (src instanceof JButton && "Crear".equals(((JButton) src).getText())) {
            if (textFieldlbFecha.getText().trim().isEmpty()) {
                new Messages(dahsboard, "Ingrese fecha").messageError();
                return;
            }
            try { 
                String fecha = textFieldlbFecha.getText().trim();
                String RUTA = "reportes/reporte_ventas_dia"  + fecha + ".pdf";
                ReporteVentasDiarias reporte = reporteVentasDiariasController.generarReporte(fecha);
                boolean ok = false;
                try {
